...
```{r}
getClonotype<-function(clonotype_index, numberA, numberB){
    result=DataFrame()
    indexInPool=sample.int(poolSize, 1, replace=TRUE)
    whichClono <- sample(nrow(Truth), 1, prob = Truth$prop)
    if (numberA>=1){
...
getSample <- function(samplesize){
    ...
    nAlpha <- rpois(samplesize, 1)
    while(any(nAlpha > 4)){
        nAlpha[nAlpha > 4] <- rpois(sum(nAlpha > 4), 1)
    }
    nBeta <- rpois(samplesize, 1)
    while(any(nBeta > 4)){
        nBeta[nBeta > 4] <- rpois(sum(nBeta > 4), 1)
    }
    barcode <- paste('cell', 1:samplesize)
    
    
    sample <- lapply(1:samplesize, function(clonotype_index){
        getClonotype(clonotype_index, nAlpha[clonotype_index], nBeta[clonotype_index])
    })
